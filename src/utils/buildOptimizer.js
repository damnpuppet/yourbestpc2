...
/**
 * weights: porcentaje de importancia de cada componente según uso
 */
const WEIGHTS = {
  ...
  export function getOptimalBuild(components, budget, useCase) {
    ...
    // Para cada tipo, calculamos score/€ ponderado
    Object.entries(components).forEach(([type, list]) => {
      // 1) computamos value = (score) / (precio) * peso
      ...
      // 2) ordenamos y escogemos el mejor
    ...
    // Si nos pasamos de presupuesto, podemos reasignar GPU o CPU a la siguiente opción
    if (totalCost > budget) {
      // estrategia sencilla: bajamos  GPU primero, luego CPU
      ['gpu','cpu'].forEach(type => {
        if (totalCost <= budget) return;
        const list = components[type];
        // obtenemos la siguiente mejor opción
        const options = list
          .map(item => {
            const price = item.price.pccomponentes + item.price.coolmod;
            const efficiency = price>0 ? item.score/price : 0;
        ...
        // find index actual y nos movemos al siguiente
